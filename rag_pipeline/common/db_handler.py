...
def delete_document_by_file_id(file_id: str) -> None:
    ...
    try:
        ...
        # Delete all document_rows with the specified file_id
        try:
            rows_response = supabase.table("document_rows").delete().eq("file_id", file_id).execute()
...
def insert_document_rows(file_id: str, rows: List[Dict[str, Any]]) -> None:
    ...
    Args:
        file_id: The Google Drive file ID (references document_metadata.file_id)
...
def process_file_for_rag(file_content: bytes, text: str, file_id: str, file_url: str, 
    ...
    try:
        ...
        text_processing = config.get('text_processing', {}) if config else {}
